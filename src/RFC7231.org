...
*注意：* 对于确定的媒体范围来说，用户代理可能被提供一个默认的质量值集合。但是，除非该用户代理是一个无法与其他[[rendering agents][渲染中介]]交互的封闭的系统，这个默认集合应该由用户来配置。
...
=From= 头字段包含有一个使用用户代理来发送请求的人类用户的一个互联网电子邮箱地址。该地址应该以[[ruby:machine-usable][机器可用]]的形式，正如[[https://tools.ietf.org/html/rfc5322#section-3.4][【RFC5322】章节 3.4]] 中的 "mailbox" 所定义的一样。
...
=Referer= 头字段让用户代理可以指定一个 URI 引用，来表明本次请求的[[ruby:target%20URI][目标 URI]] 是从哪一个资源那里获得的（也就是说，"referer" 指的是“引用页、引用者”，虽然这个字段名称存在拼写错误）。用户代理在生成 =Referer= 头字段的时候，*禁止* 包括 URI 引用里的 =fragment= 以及 =userinfo= 组件【[[https://tools.ietf.org/html/rfc3986][RFC3986]]】。 
