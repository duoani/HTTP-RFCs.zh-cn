...
=Content-Location= 头字段（[[id:91650144-4bfc-4362-b628-f96578a5c756][章节 3.1.4.2]]）与 =Location= 的不同之处在于：=Content-Location= 引用的是有效载荷内的表示形式所对应的最明确的资源。因此，在一个响应里同时包含 =Location= 和 =Content-Location= 丙个头字段是可能的。
