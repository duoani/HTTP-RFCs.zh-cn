...
新状态码的定义应该明确它是否是可缓存的。需要注意的是，所有状态码能够被缓存，仅当这个响应里带有明确的[[ruby:freshness%20information][新鲜度信息]]；但是，定义为可缓存的状态码是允许在没有明确的新鲜度信息的情况下被缓存的。同样，状态码的定义能够在缓存行为上施加约束。更多信息见【RFC7234】。
...
** 8.4. 内容编码值登记表 / Content Coding Registry
:PROPERTIES:
