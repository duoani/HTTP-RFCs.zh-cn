...
许多用于[[ruby:proactive%20negotiation][主动协商]]的请求报头域都会使用一个名叫 "q"（不区分大小写）的[[ruby:common%20parameter][通用型参数]]，来分别向内容的对应类型的[[ruby:preference][偏爱度]]赋予一个相对的“[[ruby:weight][权重值]]”。由于相同的参数名称（"q"）常常用在服务器配置里来对某个资源的各种待选表现形式所对应的质量度赋予一个权重值，因此，这个权重值被称为“[[ruby:quality%20value][质量度]]”（或者称为 =qvalue=）。
...
权重值被[[ruby:normalized][规范化]]为一个 0 到 1 范围内的实数值，其中，最小值是 0.001（优先级最低），最大值为 1（优先级最高），权重值为 0 代表“不被接受”。如果没有出现参数 "q"，那么默认的权重值为 1。 
...
发送端 *禁止* 生成小数点后超过三位小数的 =qvalue=。应该要以相同的方式来限制这些权重值的用户配置。
...
字符 "*" 用于将媒体类型归为一个范围，使用 "\ast{}/\ast{}" 来表明所有媒体类型，使用 "type/*" 来表明该 type 类型的所有子类型。=media-range= 能够包含该 range 所接收的媒体类型参数。
...
每个 =media-range= 后面可以跟随：0 个以上合适的[[ruby:media%20type%20parameters][媒体类型参数]]（比如 =charset=）；一个可选的 "q" 参数用于指明一个相对的权重值（[[id:91059de1-4a9b-4d9f-8be2-eb4d2d6ddc37][章节 5.3.1]]）；以及在这些可选项其后还可以有 0 个以上[[ruby:extension%20parameters][扩展参数]]。如果存在任何扩展（=accept-ext=），那么必须要设置 "q" 参数，这是因为它在两种参数集中扮演着一个分隔符的角色。
...
上面例子可以解释为：“我首选（偏爱） =audio/basic=，但是，如果最佳备选的质量度调低 80% 以上的话，可以发送给我任意音频类型”
...
#+BEGIN_QUOTE
译注：=text/html= 和 =text/x-c= 都没有指定 "q" 值，则默认权重值为 1（最高优先级）。
#+END_QUOTE
...
对应某个给定类型的媒体类型质量因子，是通过查找匹配该类型的最高优先级的媒体范围来确定的。
...
*注意：* 对于确定的媒体范围来说，用户代理可能被提供一个默认的质量度集合。但是，除非该用户代理是一个无法与其他渲染代理交互的封装的系统，这个默认集合应该由用户来配置。
...
字符集名称的定义见[[id:dc31dd72-3e53-4fb1-b3a6-e6716f61b202][章节 3.1.1.2]]。用户代理 *可以* 向每个字符集关联一个质量度，来表明该用户对于那种字符集的相对偏爱度，其定义见[[id:91059de1-4a9b-4d9f-8be2-eb4d2d6ddc37][章节 5.3.1]]。以下是一个例子：
...
如果 =Accept-Charset= 的域值里出现了特殊值 "*" 的话，说明匹配所有未在该域里提及到的字符集。如果 =Accept-Charset= 的域值里没有出现 "*"，说明任何没有在该域里明确提及到的字符集是不为客户端所接受的。
...
如果请求里出现了 =Accept-Charset= 报头域，但没有任何可供响应选择的[[ruby:available%20representations][备选表现形式]]所带有的字符集是被其列为可接受的，那么，源服务器可以[[ruby:honor][遵守]] =Accept-Charset= 报头域的要求而返回一个 =406 (Not Acceptable)= 响应，也可以[[ruby:disregard][忽视]]该报头域的要求就好像它并没有受到[[ruby:content%20negotiation][内容协商]]的影响一样。 
