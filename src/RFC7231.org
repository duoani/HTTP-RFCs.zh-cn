...
=100 (Continue)= 状态码表明了一个请求的[[ruby:initial%20part][初始部分]]已经被服务器接收到了，并且它还没有被服务器所[[ruby:rejected][拒绝]]。服务器打算在完全接收到这个请求并处理好之后再发送一个[[ruby:final%20response][最终响应]]。
...
=101 (Switching Protocols)= 状态码表明了服务器理解并愿意按照客户端的[[ruby:request][建议]]（该建议经由 =Upgrade= 头字段来表达，见[[id:62632d23-475d-4daa-8efb-3206d370573a][【RFC7230】章节 6.7]]）来改变本次连接上正在使用的[[ruby:application%20protocol][应用协议]]。服务器 *必须* 在响应里生成一个 =Upgrade= 头字段，来表明：从结束该 =101= 响应的空行之后开始，将会马上切换到哪一种（或多种）协议。
