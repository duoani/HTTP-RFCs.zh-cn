...
*注意：* 在 HTTP/1.0 中，=301 (Moved Permanently)= 和 =302 (Found)= 状态码是定义为上述第一种重定向类型的（[[https://tools.ietf.org/html/rfc1945#section-9.3][【RFC1945】章节 9.3]]）。早期的用户代理有一个分歧：应用到重定向目标的请求方法是使用与原来请求的方法一样呢，还是改写为 GET。
