...
不加约束的[[ruby:multiple%20range%20requests][多个部分的范围请求]]容易受到[[ruby:denial-of-service%20attacks][拒绝服务攻击]]的影响，这是因为请求相同数据的许多重合范围的工作量与试图提供所请求的许多部分的数据所消耗的时间、内存、以及宽带相比显得太微不足道了。服务器应该忽略、合并、或者拒绝
