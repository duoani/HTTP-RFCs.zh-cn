...
:PROPERTIES:
:ID:       a995c03a-651b-4bd1-9429-30f97b353acd
:END:
...
如果连接被过早地关闭了，或者请求使用了一个或多个 =Range= 规格，那么，一个响应可能只对一个表示形式的一个[[ruby:subrange][子范围]]进行传输。历经多次这样的传输以后，客户端可能已接收到同一个表示形式的许多范围。这些范围只有在它们都具有同一个[[ruby:strong%20validator][强验证器]]（[[id:d232decc-3720-4edf-bbfa-df942f60aa50][【RFC7232】章节 2.1]]）的时候才能够被安全地[[ruby:combined][结合]]起来。
...
在向一个目标资源发起多次 GET 请求后，客户端接收到多个[[ruby:partial%20responses][部分响应]]，那么，客户端 *可以* 将这些响应结合为一个大的连续的范围，如果它们共享同一个强验证器的话。
