...
一个表示形式可以依据各种结构单元来划分为多个子范围，依赖于继承自这个表示形式的媒体类型的结构。这个“[[ruby:range%20unit][范围单位]]”是用在 =Accept-Ranges= （[[id:cf601084-e3af-41bc-9ff7-8f903ca59fa8][章节 2.3]]）响应头字段里用来声明服务器支持范围请求；用在 =Range= （[[id:a70b4aaa-f776-4d5b-a31d-60d1ad16d85e][章节 3.1]]）请求头字段里用来描绘所请求的一个表示形式的哪一部分；用在 =Content-Range= （[[id:7a682301-f05e-4e79-8242-398bbfb57c74][章节 4.2]]）有效载荷头字段来描述一个表示形式的哪一部分将会被传输。
...
- The first 500 bytes (byte offsets 0-499, inclusive):
  ...
  第一个 500 字节（字节偏移量为 0-499，[[ruby:inclusive][闭合区间]]）：
...
- The second 500 bytes (byte offsets 500-999, inclusive):
  ...
  第二个 500 字节（字节偏移量为 500-999，闭合区间）：
...
客户端可以在不知道已选定的表示形式大小的情况下限制所请求的字节数量。如果缺少 =last-byte-pos= 值，或者其值大于或等于表示形式数据的当前长度，那么，字节范围会被解释为这个表示形式的剩余部分（也就是说，服务器将这个 =last-byte-pos= 的值替换为已选定表示形式的长度减 1 的值）。

#+BEGIN_QUOTE
A client can request the last N bytes of the selected representation using a =suffix-byte-range-spec=.
#+END_QUOTE

客户端可以使用一个 =suffix-byte-range-spec= 来请求已选定的表示形式的最后 N 个字节。
...
如果已选定的表示形式的长度小于 =suffix-length= 所指定的值，表示使用整个表示形式。
...
- The final 500 bytes (byte offsets 9500-9999, inclusive):
  ...
  最后 500 字节（字节偏移量为 9500-9999，闭合区间）：
...
- Other valid (but not canonical) specifications of the second 500 bytes (byte offsets 500-999, inclusive):
  ...
  其他有效的（但[[ruby:not%20canonical][不规范的]]）对第二个 500 字节的指定（字节偏移量为 500-999，闭合区间）：
