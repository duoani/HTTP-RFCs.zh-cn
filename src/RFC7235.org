...
超文本传输协议（HTTP）是一种[[ruby:stateless][无状态]]的应用层协议，适用于分布式、协作式的超文本信息系统。本文档定义了 HTTP 的[[ruby:Authentication%20framework][认证框架]]。
...
HTTP 提供了一个通用的访问控制和认证框架，经由一个可扩展的[[ruby:challenge-response][质询-作答]]认证方案的集合，服务器可以使用这种认证方案对客户端的请求进行质询，而客户端可以使用这种谁方案来向服务器提供认证信息。本文档依据《超文本传输协议（HTTP/1.1）：消息与路由》【[[file:RFC7230.org][RFC7230]]】所定义的架构来定义了 HTTP/1.1 的认证，包括：之前在《HTTP 认证：基础与数字访问认证》【[[https://tools.ietf.org/html/rfc2617][RFC2617]]】有所描述的通用框架，以及之前在《超文本传输协议——HTTP/1.1》【[[https://tools.ietf.org/html/rfc2616][RFC2616]]】所定义的相关的字段和状态码。
...
IANA 认证方案登记表（[[id:af1963ff-7886-492d-93cd-3d76aaf2d475][章节 5.1]]）列出了已登记的认证方案及其对应的规范，包括之前由 RFC 2617 所定义的 "basic" 和 "digest" 认证方案。
...
HTTP 提供了一种简单的[[ruby:challenge-response][质询-作答]]认证框架，服务器可以使用它来对客户端请求进行质询，而客户端可以使用它来提供认证信息。它使用一个不区分大小写的[[ruby:token][标记]]来作为一种标识[[ruby:authentication%20scheme][认证方案]]的方法，这个标记之后紧跟的是通过这种方案来获取认证所必要的额外信息。这些额外信息要么是一个以逗号分隔的参数列表，要么是一个可以持有 base64 编码信息的字符序列。
...
认证参数是 =name=value= 键值对，其中 =name= 标记是按不区分大小写的方式来匹配的，并且在每一次质询当中，每个参数名称 *必须* 仅出现一次。
...
源服务器使用一个 =401 (Unauthorized)= 响应消息来向用户代理质询认证，其中带有一个 =WWW-Authenticate= 头字段包含至少一个适用于所要请求的资源的质询。
...
代理使用一个 =407 (Proxy Authentication Required)= 响应消息来向客户端质询认证，其中带有一个 =Proxy-Authenticate= 头字段包含至少一个适用于代理去请求资源的质询。
...
在接收到一个 =401 (Unauthorized)= 以后，如果用户代理希望通过源服务器来认证自身（通常但并非必然），可以通过在请求中带有一个 =Authentication= 头字段来达到。
...
在接收到一个 =407 (Proxy Authentication Required)= 以后，如果用户代理希望通过代理来认证自身（通常但并非必然），可以通过在请求中带有一个 =Proxy-Authorization= 头字段来达到。
...
如果源服务器接收到一个访问保护资源的请求，一旦这个请求缺少凭证，或者包含有一个无效的凭证（比如，一个错误的密码），或者不完整的凭证（比如，当认证方案要求超过一个以上回合的时候），那么，源服务器 *应当* 发送一个 =401 (Unauthorized)= 响应，其内包含有一个 =WWW-Authenticate= 头字段，该字段至少带有一个（可能是新的）适用于所请求的资源的质询。
...
同样，如果代理接收到一个请求，一旦这个请求缺少代理凭证，或者包含有无效或不完整的代理凭证，那么，要求认证的代理 *应当* 生成一个 =407 (Proxy Authentication Required)= 响应，其内包含有一个 =Proxy-Authenticate= 头字段，该字段至少带有一个（可能是新的）适用该代理的质询。
...
HTTP 并没有将对这个简单的[[ruby:challenge-response][质询-作答]]框架上的应用限定为访问认证。可以使用其他额外的机制（比如在传输层上的认证，或者经由消息封装的认证），以及带有额外头字段所指定的认证信息。然而，本规范并没有定义这些额外的机制。
...
=realm= 是一个保留的认证参数，某些[[ruby:authentication%20schemes][认证方案]]如果希望去表明保护范围的话，可以使用它来实现目的。
...
[[ruby:protection%20space][保护空间]]是由被访问的服务器的[[ruby:canonical%20root%20URI][规范化根 URI]]（即实际请求 URI 的 =scheme= 和 =authority= 组件，见[[id:3265c21d-0d3b-4776-8e28-38278d168779][【RFC7230】章节 5.5]]），并结合 =realm= 的值（如果有出现的话）来进行定义的。这些[[ruby:realms][领域]]使得在服务器上要保护的各种资源可以划分为一系列的[[ruby:protection%20spaces][保护空间]]，每一个保护空间都有自己的认证方案以及/或者认证数据库。=realm= 的值是一个字符串，通常由源服务器所赋值，可以具有对应认证方案所特有的额外语义。需要注意的是，一个响应可以有多个具有相同 =auth-scheme= 但不同 =realm= 的质询。
...
保护空间决定了哪一种凭证可以被自动执行。某个之前的请求假如已经经过授权，那么，对于这个保护空间内的所有其他请求，用户代理 *可以* 在一段时间内复用同一个凭证，而这个具体时间为多久，取决于[[ruby:authentication%20scheme][认证方案]]、参数，以及/或者用户的偏好设置（比如一个可配置的闲置超时时间）。除非认证方案明确允许，单个保护空间的范围不能超出它的服务器的范围。
...
** 5.1. 认证方案登记表 / Authentication Scheme Registry
:PROPERTIES:
...
*** 5.1.2. 新的认证方案的注意事项 / Considerations for New Authentication Schemes
