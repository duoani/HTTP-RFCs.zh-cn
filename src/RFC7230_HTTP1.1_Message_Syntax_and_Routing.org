...
HTTP/1.1 仍保留了对 HTTP/1.0 的兼容：通过包含更多严格的要求来让实现更加可靠，仅添加那些能够被 HTTP/1.0 接收端安全忽略的，或者只能在通信双方都声明其遵循 HTTP/1.1 语义的时候才会使用的功能。
...
*** A.1.1. 多宿主网页服务器 (Multihomed Web Servers)
#+BEGIN_QUOTE
...
旧的 HTTP/1.0 客户端假设 IP 地址与服务器是一个一对一关系，除了只能使用 IP 地址这种方式来将一个请求导向到它所期望到达的服务器以外，没有任何其他方式。在 HTTP/1.1 发展过程中引入了 =Host= 报头域，它能够很快被 HTTP/1.0 浏览器所实现，为了确保，额外的要求被附加到 HTTP/1.1 请求里
