...
TODO 正如[[id:e2d9bed8-d6ca-4b5d-ab4a-6b2b2576393e][章节 2.3]] 所述，中间人能够在处理 HTTP 请求和响应中饰演多种角色。某些中间人是用于提升性能或可用性，另外一些用于访问控制（Access Control）或者内容过滤（Filter Content）。因为一个 HTTP 流带有类似于一个[[https://en.wikipedia.org/wiki/Pipeline_(software)][管道架构（Pipe-and-Filter Architecture）]]的性质，因此，对中间人能够提升（或干扰）流的各个方向的程序没有固定限制。
...
中间人 *禁止* 转发一个报文到自身，除非它是受到无限请求循环保护。通常情况下，中间人应该了解它自身的服务器名称，包括任何别名、局部变种、文本性的 IP 地址，并将这些信息直接响应到这种请求中去。
...
=Via= 报头域代表中间人协议的出现，并且在用户代理和服务器之间（关于请求报文）或者源服务器和客户端之间（关于响应）的接收端，
