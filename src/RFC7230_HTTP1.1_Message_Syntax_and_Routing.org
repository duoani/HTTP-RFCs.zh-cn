...
TODO 当报文发送到接收端但没有指明其支持更高的版本，接收端可以假设这个报文带有更高的次版本，为所有具有相同主版本的实现（Implementations）去安全处理这些报文提供足够的向后兼容性。
...
HTTP 报文能够解析（Parse）为用于增量处理或转发到下游（Forwarding Downstream）的流。但是，接收端不能依赖局部报文的增量投递，因为某些实现（Implementations）会因为网络性能、安全校验或者有效载荷转换（[[id:b6641b58-3ac6-4fce-9c44-a3715ffdc8a0][Payload Transformation]]）而对这些不完整的报文进行缓冲或延迟转发。
