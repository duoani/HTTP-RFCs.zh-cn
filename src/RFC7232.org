...
=304 (Not Modified)= 状态码表明：已接收到 GET 或 HEAD 条件请求，如果这个条件的求值结果不成立的话，将返回的是一个 =200 (OK)= 响应。换句话说，服务器不需要传输目标资源的一个表示形式，因为这个请求表明了发起这个条件请求的客户端已经有一个有效的表示形式了，因此服务器让客户端重新使用那个已缓存的表示形式，可以把它当作是一个 =200 (OK)= 响应的有效载荷。
