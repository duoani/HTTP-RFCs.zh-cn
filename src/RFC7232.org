...
=If-Match= 头字段使得请求方法带有条件，接收端服务器仅在满足以下条件之一时才会返回资源：
...
源服务器 *禁止* 执行请求方法，如果所接收到的 =If-Match= 条件的求值结果为否的话。而是，源服务器 *必须* 响应以下其中之一：

1) =412 (Precondition Failed)= 状态码，或者
2) =2xx (Successful)= 状态码的其中之一，如果源服务器已证实出目标资源又被请求了一种状态改变，并且它的最终状态已经反映为目标资源当前状态（也就是说，用户代理所请求的改变已经成功完成了，但是用户请求可能没有意识到这个，可能是因为之前的响应已经丢失了没有成功到达用户代理，或者其他某些用户代理也对目标资源请求了一种与之兼容的变化）。 

在其他情况下，源服务器 *禁止* 发送一个验证器头字段到响应里，除非它能够证实这个请求与对应的同一个用户代理所造成的前一个相邻的变化相重复。
...
=If-None-Match= 头字段使得请求方法带有条件，接收端缓存服务器或源服务器仅在满足以下条件之一时才会返回资源：
...
=If-None-Match= 主要用在条件 GET 请求中以启用缓存信息的有效更新
