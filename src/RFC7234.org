...
=no-store= 请求指令表明，缓存 *禁止* 存储这次请求以及对应的所有响应的任何部分。本指令既适用于[[ruby:private%20caches][私有缓存]]，也适用于[[ruby:shared%20caches][共享缓存]]。这里的“禁止存储”意思是缓存 *禁止* 在[[ruby:non-volatile%20storage][非易失性存储器]]中刻意存储信息，而且 *必须* 在转发这些信息以后，立刻、尽最大努力地去尝试从[[ruby:volatile%20storage][易失性存储器]]中移除它们。
...
如果 no-cache 响应指令指定了一个或多个字段名，那么，缓存 *可以* 使用
