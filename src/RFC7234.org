...
- =Authorization= 头字段（[[id:14671c8e-e5a4-4f9d-ad1d-6460328ed7f5][【RFC7235】章节 4.2]]）没有出现在请求里，如果这是个共享缓存的话，除非响应显式允许它（[[id:d6b247ad-fe70-4ddf-9bd3-ebc35ea6ac99][章节 3.2]]），并且：
- 响应符合以下情况之一：
...
需要注意的是，在常规操作中，如果一个响应既没有[[ruby:cache%20validator][缓存验证器]]也没有一个显式的过期时间，某些缓存将不会存储这种响应，因为存储这种响应通常是没有用处的。但是，缓存不会被禁止存储这种响应。
...
#+BEGIN_QUOTE
译注：所谓 "serve stale"，指的是：提供陈旧的缓存响应来作为响应，即使这个已缓存的响应已经过期。对于一个请求，当对应的已缓存的响应已经过期（stale），并且源服务器无响应或者返回一个错误，这时候，缓存服务器返回这个 stale 的响应来作为回应，而不是返回错误。在大多数情况下，与其响应超时或返回其他错误，倒不如返回给用户一个能用的但过期的响应。
#+END_QUOTE
...
- 使用新响应所提供的其他头字段，除以 =Content-Range= 以外，来替换所有在已存储的响应对应的头字段。
...
响应的[[ruby:freshness%20lifetime][保鲜期]]是从它被源服务器生成开始到它的[[ruby:expiration%20time][过期时间]]之间的时间长度。一个[[ruby:explicit%20expiration%20time][显式的过期时间]]，是源服务器打算让一个已存储的响应，在没有进一步的验证的情况下，不再能够被缓存所使用的那个时间点。与之相对，一个[[ruby:heuristic%20expiration%20time][启发式的过期时间]]，是当没有可用的显式的过期时间的时候，由缓存所分配的。
...
一个 "stale"（陈旧的、不新鲜的）的响应是这样一种响应：或者带有显式的[[ruby:expiry%20information][过期信息]]或者允许去进行启发式的[[ruby:expiry%20calculated][过期计算]]，但根据[[id:d414d2f7-d865-4b94-9efd-6b2578df86aa][章节 4.2]] 的计算，已不再[[ruby:fresh][新鲜]]。
